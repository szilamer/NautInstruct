...
import type { DiagnosisQuality } from '../content/types'
...
export function DebriefPanel() {
  ...
  const submitDiagnosis = useStore((s) => s.submitDiagnosis)
  ...
  if (!session || !situation || !session.lastEval) return null
  const useLlm = hasApiKey(llmSettings)
  const { debrief, debriefLoading, lastEval } = session
    ...
    <div className="absolute inset-0 flex items-center justify-center bg-black/60 p-4">
      <div className="max-h-full w-full max-w-xl overflow-y-auto rounded-2xl border border-white/10 bg-sea-900 p-6 shadow-2xl">
        <div className="flex items-center gap-2 text-buoy-red">
          ...
          <h3 className="text-lg font-bold">A szimulátor megállt – hiba történt</h3>
        </div>
        <p className="mt-2 text-sm text-foam/70">
          Szabály: <b>{lastEval.rule.title}</b>. Mielőtt elárulnám a megoldást: gondold át, mit
          rontottál el.
        </p>

        {!debrief && (
          <div className="mt-4">
            <div className="font-semibold">„Szerinted mit rontottál el?"</div>
            <div className="mt-3 space-y-2">
              {situation.diagnosisOptions.map((o) => (
                <button
                  key={o.id}
                  onClick={() => setSelected(o.id)}
                  className={`block w-full rounded-lg border px-4 py-2 text-left text-sm transition ${
                    selected === o.id
                      ? 'border-sea-500 bg-sea-500/20'
            ...
            {useLlm && (
              <div className="mt-4">
                <label className="text-xs text-foam/60">
                  Vagy fogalmazd meg saját szavaiddal (AI értékeli):
                </label>
                <textarea
                  value={freeText}
                  ...
                  placeholder="Pl. a sodrással megegyező irányba fordultam…"
                />
              </div>
            )}

            <button
              disabled={debriefLoading || (!selected && !freeText.trim())}
              onClick={() => submitDiagnosis({ diagnosisOptionId: selected, freeText: freeText.trim() || undefined })}
            ...
            >
              {debriefLoading ? 'Értékelés…' : 'Diagnózis elküldése'}
            </button>
          </div>
        )}
          ...
          <div className="mt-4">
            <span className={`inline-block rounded-full border px-3 py-1 text-xs font-semibold ${qualityStyle[debrief.quality].cls}`}>
              ...
              <span className="ml-2 opacity-60">
                {debrief.source === 'llm' ? 'AI-magyarázat' : 'sablon'}
              </span>
            </span>
            <p className="mt-3 whitespace-pre-line text-sm leading-relaxed text-foam/90">
              {debrief.explanation}
            </p>
            <div className="mt-3 rounded-lg bg-sea-800/70 p-3 text-xs text-foam/70">
              Vizsganyelven: <b>{lastEval.rule.correctSummary}</b>
            </div>
